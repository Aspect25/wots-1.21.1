...
public class UziVariantWheelScreen extends Screen {

    private final BlockPos targetPos;
    private static final UziPlushVariant[] VARIANTS = UziPlushVariant.values();
    ...
    public UziVariantWheelScreen(BlockPos pos) {
        super(Text.empty());
        this.targetPos = pos;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // Click centre disc = lazy mode
        if (dx*dx + dy*dy <= INNER_R * INNER_R) {
            ClientPlayNetworking.send(new SetUziVariantPayload(targetPos, "LAZY", true));
            close();
            return true;
        }

        // Click segment = pick variant
        if (button == 0 && hoveredIndex >= 0) {
            ClientPlayNetworking.send(new SetUziVariantPayload(targetPos, VARIANTS[hoveredIndex].name(), false));
            close();
            return true;
        }
