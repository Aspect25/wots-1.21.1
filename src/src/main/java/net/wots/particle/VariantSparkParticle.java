...
/** Colored fast spark for variant change effect. */
@Environment(EnvType.CLIENT)
public class VariantSparkParticle extends AbstractSolverParticle {
    protected VariantSparkParticle(ClientWorld w, double x, double y, double z, double vx, double vy, double vz, SpriteProvider sp) {
        super(w,x,y,z, VariantParticleColor.r(), VariantParticleColor.g(), VariantParticleColor.b(), sp);
        maxAge=6+w.random.nextInt(8); scale=0.04f+w.random.nextFloat()*0.04f;
        ...
        red=1f;green=0.95f;blue=1f; setSpriteForAge(sp);
    }
    ...
    @Environment(EnvType.CLIENT) public static class Factory implements ParticleFactory<SimpleParticleType>{
        ...
        @Override public Particle createParticle(SimpleParticleType t,ClientWorld w,double x,double y,double z,double vx,double vy,double vz){
            return new VariantSparkParticle(w,x,y,z,vx,vy,vz,sp);}
    }
}
